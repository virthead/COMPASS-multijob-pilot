...
def getNewJob(tofile=True, num_of_job = 300):
    ...
    if num_of_job > 1:
        if 'HPC_Titan' in pUtil.readpar("catchall"):
            nodes, walltime, default = get_hpc_resources(max_nodes=num_of_job, min_nodes = 15, min_walltime=90)
        else:
            nodes, walltime, default = get_hpc_resources(max_nodes=num_of_job, min_nodes = 1, min_walltime=90)
    ...
    if not env['jobRequestFlag']:
        ...
        jobs = []
        t_start = time.time()
        while len(jobs) < num_of_job:
            num_of_job_to_get = num_of_job - len(jobs)
            # construct a dictionary for passing to jobDispatcher and get the prodSourceLabel
            jNode, prodSourceLabel, pilotErrorDiag = getDispatcherDictionary(_diskSpace, tofile, num_of_job_to_get)
            if jNode == {}:
                errorText = "!!FAILED!!1200!! %s" % (pilotErrorDiag)
                pUtil.tolog(errorText, tofile=tofile)
                # send to stderr
            ...
            if StatusCode != '0':
                if len(jobs) == 0:
                    ...
                    pUtil.tolog("No job received from jobDispatcher, continue with %s collected jobs" % (len(jobs)))
                    break

            data = ret[1]
            response = ret[2]   # text
            
            jobs_get = json.loads(data['jobs'])
            jobs = jobs + jobs_get
            pUtil.tolog("Collected %s jobs" % (len(jobs)))
