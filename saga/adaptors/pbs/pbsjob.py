...
def _pbscript_generator(url, logger, jd, ppn, gres, pbs_version, is_cray=False, queue=None, ):
    ...
    if (is_cray is "") or not('Version: 4.2.7' in pbs_version) or not('Version: 6.0.4' in pbs_version):
...
#        pbs_params += "#PBS -d %s \n" % jd.working_directory
    ...
    if is_cray is not "":
        ...
        elif 'Version: 6.' in pbs_version:
            logger.info("Using TORQUE 6.x notation '#PBS -l procs=XX' ")
            pbs_params += "#PBS -l flags=commtransparent,procs=%d\n" % jd.total_cpu_count
        else:
